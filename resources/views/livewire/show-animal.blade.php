                ...
                <div class="p-6 md:flex md:items-center md:space-x-6">
                    ...
                    <div class="mt-4 md:mt-0 text-center md:text-left flex-grow">
                        ...
                        <div class="mt-2 text-sm text-gray-600">
                            ...
                            <span>{{ \Carbon\Carbon::parse($animal->data_nascimento)->age }} anos</span><span class="mx-2">&bull;</span>
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 text-gray-900">
                            ...
                            <div class="mt-4 border-t border-gray-200">
                                <dl>
                                    <div class="bg-gray-50 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt class="text-sm font-medium text-gray-500">Localização</dt><dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $animal->localizacao->nome ?? 'Não definida' }}</dd></div>
                                    <div class="bg-white px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt class="text-sm font-medium text-gray-500">Categoria</dt><dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $animal->categoria->nome ?? 'Não definida' }}</dd></div>
                                    <div class="bg-gray-50 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt class="text-sm font-medium text-gray-500">Pai</dt><dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">@if($animal->pai)<a href="{{ route('animais.show', $animal->pai) }}" class="text-if-green-600 hover:underline">{{ $animal->pai->identificacao }}</a>@else Não identificado @endif</dd></div>
                                    <div class="bg-white px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt class="text-sm font-medium text-gray-500">Mãe</dt><dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">@if($animal->mae)<a href="{{ route('animais.show', $animal->mae) }}" class="text-if-green-600 hover:underline">{{ $animal->mae->identificacao }}</a>@else Não identificada @endif</dd></div>
                                    @if($animal->observacoes)<div class="bg-gray-50 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt class="text-sm font-medium text-gray-500">Observações</dt><dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $animal->observacoes }}</dd></div>@endif
                                </dl>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 text-gray-900">
                            @if (session()->has('sucesso'))
                                <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-3 mb-4 rounded" role="alert"><p>{{ session('sucesso') }}</p></div>
                            @endif
                            ...
                            <form wire:submit="salvarMovimentacao" class="mt-4 border-t border-gray-200 py-6 space-y-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div><label for="tipo" class="block text-sm font-medium text-gray-700">Tipo de Evento</label><select wire:model.live="tipo" id="tipo" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"><option>Observação</option><option>Pesagem</option><option>Vacinação</option><option>Medicação</option><option>Venda</option><option>Óbito</option></select>@error('tipo')<span class="text-red-500 text-xs mt-1">{{ $message }}</span>@enderror</div>
                                </div>
                                @if ($tipo === 'Pesagem')
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        ...
                                        <div><label for="unidade" class="block text-sm font-medium text-gray-700">Unidade</label><select wire:model="unidade" id="unidade" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"><option>Kg</option><option>@</option></select></div>
                                    </div>
                           ...
                           <div class="mt-6 border-t border-gray-200 pt-6">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="hidden sm:block"><div class="border-b border-gray-200"><nav class="-mb-px flex space-x-8 overflow-x-auto" aria-label="Tabs">@foreach (['Reprodutivo', 'Agenda Sanitária', 'Todos', 'Pesagem', 'Vacinação', 'Medicação', 'Observação'] as $tab)<button wire:click="$set('activeTab', '{{ $tab }}')" @class(['whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm', 'border-if-green-500 text-if-green-600' => $activeTab === $tab, 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' => $activeTab !== $tab])>{{ $tab }}</button>@endforeach</nav></div></div>
                                </div>
                                <div class="mt-6 flow-root">
                                   <ul role="list" class="-mb-8">
                                        @if($activeTab === 'Reprodutivo')
                                            @forelse ($animal->eventosReprodutivos as $evento)
                                                <li><div class="relative pb-8">
                                                    @if (!$loop->last)<span class="absolute top-4 left-4 -ml-px h-full w-0.5 bg-gray-200"></span>@endif
                                                    <div class="relative flex items-start space-x-3">
                                                        <div><span class="h-8 w-8 rounded-full bg-if-green-500 flex items-center justify-center ring-8 ring-white"><i class="fas {{ $this->getIconeEventoReprodutivo($evento->tipo) }} text-white"></i></span></div>
                                            ...
                                            @forelse ($animal->agendaSanitaria->sortBy('data_agendada') as $evento)
                                                <li><div class="relative pb-8">
                                                    ...
                                                    <div class="relative flex items-start space-x-3">
                                                        ...
                                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                            <div><p class="text-sm font-medium text-gray-900">{{ $evento->protocoloEvento->nome_evento }}</p><p class="text-sm text-gray-500">{{ $evento->protocoloEvento->instrucoes }}</p></div>
                                            ...
                                            @forelse ($movimentacoesFiltradas as $movimentacao)
                                                <li><div class="relative pb-8">
                                                    ...
                                                    <div class="relative flex items-start space-x-3">
                                                        ...
                                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                            <div><p class="text-sm text-gray-500"><span class="font-medium text-gray-900">{{ $movimentacao->tipo }}:</span> {{ $movimentacao->descricao }} @if($movimentacao->valor)<span class="font-semibold">({{ $movimentacao->valor }})</span>@endif</p></div>
                                                            <div class="text-right text-sm whitespace-nowrap text-gray-500"><time>{{ \Carbon\Carbon::parse($movimentacao->data)->format('d/m/Y') }}</time><div class="mt-1"><button wire:click="startEditing({{ $movimentacao->id }})" class="text-blue-500 hover:text-blue-700" title="Editar"><i class="fas fa-edit"></i></button><button wire:click="deleteMovimentacao({{ $movimentacao->id }})" wire:confirm="Tem a certeza?" class="ml-2 text-red-500 hover:text-red-700" title="Remover"><i class="fas fa-trash"></i></button></div></div>
                                                        </div>
                                                    </div>
                                                </div></li>
                                            @empty
